...
async def file_cloner(client, message):
    print(f"📂 Nayi File Detect Hui: {SOURCE_CHANNEL}")
    for chat in DESTINATIONS:
        try:
            await message.copy(chat_id=chat, caption="") 
            print(f"✅ File copy ho gayi: {chat}")
        except Exception as e:
            print(f"❌ {chat} mein error: {e}")

if __name__ == "__main__":
    print("🚀 Web Server Starting...")
    Thread(target=run_web).start() # Yeh Render ko zinda rakhega
